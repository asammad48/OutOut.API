...
namespace OutOut.Persistence.Identity.Stores
{
        ...
        where TRole : ApplicationRole
    {
        ...
        public UserStore(ApplicationNonSqlDbContext nonSqlContext, IRoleStore<TRole> roleStore, ILookupNormalizer normalizer)
        {
        ...
        public IQueryable<TUser> Users
        {
            get
            {
                var query = _nonSqlContext.GetCollection<TUser>().AsQueryable();
                return query;
            }
        }
        ...
        private async Task Update<TFIELD>(TUser user, Expression<Func<TUser, TFIELD>> expression, TFIELD value)
        {
            ...
            await _nonSqlContext.GetCollection<TUser>().UpdateOneAsync(x => x.Id == user.Id, upd);
        }

        private async Task RemoveAll<TFIELD>(TUser user, Expression<Func<TUser, IEnumerable<TFIELD>>> expression, IEnumerable<TFIELD> values)
        {
            var upd = Builders<TUser>.Update.PullAll(expression, values);
            await _nonSqlContext.GetCollection<TUser>().UpdateOneAsync(x => x.Id == user.Id, upd);
        }
        private async Task RemoveAll<TFIELD>(TUser user, Expression<Func<TUser, IEnumerable<TFIELD>>> expression, Expression<Func<TFIELD, bool>> filter)
        {
            var upd = Builders<TUser>.Update.PullFilter(expression, filter);
            await _nonSqlContext.GetCollection<TUser>().UpdateOneAsync(x => x.Id == user.Id, upd);
        }
        ...
        private Task<TUser> ById(string id) => _nonSqlContext.GetCollection<TUser>().FirstOrDefaultAsync(x => x.Id == id);
        ...
        public async Task<IdentityResult> CreateAsync(TUser user, CancellationToken cancellationToken)
        {
            ...
            var u = await _nonSqlContext.GetCollection<TUser>().FirstOrDefaultAsync(x => x.UserName == user.UserName);
            if (u != null) return IdentityResult.Failed(new IdentityError { Code = "Username already in use" });

            await _nonSqlContext.GetCollection<TUser>().InsertOneAsync(user, InsertOneOptions, cancellationToken);
            if (user.Email != null)
            {
        ...
        public async Task<IdentityResult> DeleteAsync(TUser user, CancellationToken cancellationToken)
        {
            ...
            await _nonSqlContext.GetCollection<TUser>().DeleteOneAsync(x => x.Id == user.Id, cancellationToken);
            return IdentityResult.Success;
        }
        ...
        public Task<TUser> FindByNameAsync(string normalizedUserName, CancellationToken cancellationToken)
        {
            ...
            return _nonSqlContext.GetCollection<TUser>().FirstOrDefaultAsync(x => x.NormalizedUserName == normalizedUserName);
        }
        ...
        public async Task<IdentityResult> UpdateAsync(TUser user, CancellationToken cancellationToken)
        {
            ...
            await SetEmailAsync(user, user.Email, cancellationToken);
            await _nonSqlContext.GetCollection<TUser>().ReplaceOneAsync(x => x.Id == user.Id, user, ReplaceOptions, cancellationToken);
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            var result = await _nonSqlContext.GetCollection<TUser>().FindAsync(u => u.Claims.Any(c => c.ClaimType == claim.Type && c.ClaimValue == claim.Value));
        ...
        public async Task<TUser> FindByEmailAsync(string normalizedEmail, CancellationToken cancellationToken)
        {
            ...
            return await _nonSqlContext.GetCollection<TUser>().FirstOrDefaultAsync(a => a.NormalizedEmail == normalizedEmail);
        }
        ...
        public async Task<TUser> FindByLoginAsync(string loginProvider, string providerKey, CancellationToken cancellationToken)
        {
            ...
            return await _nonSqlContext.GetCollection<TUser>().FirstOrDefaultAsync(u =>
        ...
        public async Task<IList<TUser>> GetUsersInRoleAsync(string roleName, CancellationToken cancellationToken)
        {
            ...
            return (await _nonSqlContext.GetCollection<TUser>().FindAsync(filter, new FindOptions<TUser>(), cancellationToken)).ToList();
        }
        ...
        public async Task<List<IdentityRefreshToken>> GetRefreshTokensAsync(string userId, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var dbUser = await ById(userId);
            return dbUser?.RefreshTokens?.ToList() ?? new List<IdentityRefreshToken>();
        }

        public async Task RemoveRefreshTokenAsync(TUser user, string Token, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            await RemoveAll(user, x => x.RefreshTokens, x => x.RefreshToken == Token);
        }

        public async Task AddRefreshTokenAsync(TUser user, string Token, DateTime ExpirationDate, string AccessTokenUniqeId, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            await Add(user, x => x.RefreshTokens, new IdentityRefreshToken()
            {
                RefreshToken = Token,
                AccessTokenUniqeId = AccessTokenUniqeId,
                ExpirationDate = ExpirationDate
            });
        }

        public async Task RemoveAllExpiredRefreshTokensAsync(TUser user, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            await RemoveAll(user, x => x.RefreshTokens, x => x.ExpirationDate < DateTime.UtcNow);
        }
    }
}
